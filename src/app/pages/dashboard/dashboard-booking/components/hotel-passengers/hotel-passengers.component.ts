...
export class HotelPassengersComponent implements OnInit, OnDestroy  {
  ...
  initializeForm() {
    this.searchpaxForm = this.fb.group({
      searchArrayData: this.fb.array([], [Validators.required])
    });
  }
  get d() {
    ...
    if( this.updatedata?.length>0){
      for (let i = 0; i < this.updatedata?.length; i++) {
        ...
        ((this.searchpaxForm.get('searchArrayData') as FormArray).at(i) as FormGroup)?.patchValue({
          ...
          passengerTitle: this.updatedata[i]?.prefix === '' ? '':this.updatedata[i]?.prefix,
          ...
          passengerNationality: this.updatedata[i]?.nationality === ''|| this.updatedata[i]?.nationality === 0 ? '':this.updatedata[i]?.nationality,
          //passengerNationality: nalitionaliyname,
          passengerDob: this.datepipe.transform(this.updatedata[i]?.dob, 'yyyy-MM-dd'),
          passport: this.updatedata[i]?.passport,
          issuedCountry: this.updatedata[i]?.issuedCountry === ''  || this.updatedata[i]?.issuedCountry === 0? '':this.updatedata[i]?.issuedCountry,
          //issuedCountry: countryname,
          //passengerType: this.updatedata[i]?.paxType === '' ? '':this.updatedata[i]?.paxType,
        });
        //console.log(this.searchpaxForm.value.searchArrayData);
  ...
  */
    ...
    if(this.searchArrayData.value.length>0){
      ...
      if(SELECTED_PAX?.length===0){
        ...
        const SelectedPersons={
          passengers: SELECTED_PAX,
          roomIndex: this.roomIndex
        };

        this.SelectedPassengersService.sendHotelData(SelectedPersons);
        this.resultShow = false;
         this.cdr.markForCheck();
      }
     }
  ...
  onSelectPaxType(index:number,value:string){

   const PAX_TYPE_NAME=this.masterPaxTypeList.find((v)=>v.id===Number(value));

   if(PAX_TYPE_NAME){
    ((this.searchpaxForm.get('searchArrayData') as FormArray).at(index) as FormGroup).patchValue({
      passengerAttr1:PAX_TYPE_NAME?.name
     });
   }else{
    ((this.searchpaxForm.get('searchArrayData') as FormArray).at(index) as FormGroup).patchValue({
      passengerAttr1:null
  ...
  onSelectEditPaxType(index:number,value:string){
    const PAX_TYPE_NAME=this.masterPaxTypeList.find((v)=>v.id===Number(value));
    if(PAX_TYPE_NAME){
     ((this.editresultForm.get('editArrayData') as FormArray).at(index) as FormGroup).patchValue({
      passengerAttr1:PAX_TYPE_NAME?.name
      });
    }else{
     ((this.editresultForm.get('editArrayData') as FormArray).at(index) as FormGroup).patchValue({

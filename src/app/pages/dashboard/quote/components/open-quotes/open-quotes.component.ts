...
import { fromEvent, Observable, of, OperatorFunction, Subject, Subscription } from 'rxjs';
...
@Component({
  ...
  styleUrls: ['./open-quotes.component.scss','../../../../../../assets/sass/libs/datatables.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class OpenQuotesComponent implements OnInit, OnDestroy   {
  ...
  constructor(
    ...
    this.titleService.setTitle('Quotes Open List');
...
onSearchForm(searchObj){
  this.quotesServices.getquotes(searchObj, QuotesConstants.quotes_search.openQuotes).pipe(takeUntil(this.ngDestroy$)).subscribe((res) => {
...
/*   redirectQuote(srId){
  ...
  redirectQuote(srId){
    if(srId){
     const redirectRfqUrl = this.router
     .createUrlTree(['/dashboard/quotes/ancillary-quote'],{
      queryParams: { sr_id: srId },
    }).toString();
    // alert(redirectRfqUrl);

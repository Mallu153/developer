...
import { HotelRfqPricePopUpComponent } from '../hotel-rfq-price-pop-up/hotel-rfq-price-pop-up.component';

@Component({
  selector: 'app-hotel-rfqs-awt-responses',
  templateUrl: './hotel-rfqs-awt-responses.component.html',
  styleUrls: ['./hotel-rfqs-awt-responses.component.scss']
})
export class HotelRfqsAwtResponsesComponent implements OnInit, OnDestroy {
...
Supplierformatter = (supplier: Supplier) => supplier.businessName;
 ...
 constructor(
  ...
  this.titleService.setTitle('Hotel-RFQs-Awaiting Supplier Response');
...
numberOnly(event): boolean {
  ...
  getSupplierSerachData(searchObj){
      this.rfqServices.RFQFlightSearch(searchObj, RFQURLS.HOTEL_RFQ_LIST.RfqRelationRelationSearch).pipe(takeUntil(this.ngDestroy$)).subscribe((res:any) => {
       /*  if(res.message){
  ...
  openHotelRequest(sr:number,srLine:number,contact:number){
    if(sr&&srLine&&contact){
      this.router.navigate(['/dashboard/booking/hotel'], {
        queryParams: { requestId:sr, contactId: contact,hotelLineId:srLine },
      });
    }

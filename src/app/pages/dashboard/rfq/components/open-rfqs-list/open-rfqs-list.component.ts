...
@Component({
  selector: 'app-open-rfqs-list',
  templateUrl: './open-rfqs-list.component.html',
  styleUrls: ['./open-rfqs-list.component.scss']
})
export class OpenRfqsListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.titleService.setTitle('Open RFQ List');
  }
 ...
 customerDependsOnContactList(customerNumber: number) {
   this.rfqServices.getContactData(customerNumber, RFQURLS.RFQ_List.contact).pipe(takeUntil(this.ngDestroy$)).subscribe((data: any) => {
     if(data){
       ...
       this.toastrService.error('Oops! Something went wrong  while fetching the Customer contact data please try again', 'Error');
     }

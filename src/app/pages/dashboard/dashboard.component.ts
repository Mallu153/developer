...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  pageAccess = PERMISSION_KEYS.NAV_BAR;
  navBarkeys=[];
  constructor(
 ...
 async  getNavbarAcess() {
    const USER_ROLE_NUMBER = this.authService.getUserRoleNumber();
      try {
        const response = await this.premissionsServices.getPagePermissions(this.pageAccess.NAV_BAR_PAGE_NUMBER, USER_ROLE_NUMBER);
        if (response.length > 0) {
          this.navBarkeys=response;
          this.cdr.markForCheck();
        } else {
          this.navBarkeys=[];
          this.toastr.warning('No page key found', 'Warning', { progressBar: true });
          this.cdr.markForCheck();
        }
      } catch (error) {
        this.navBarkeys=[];

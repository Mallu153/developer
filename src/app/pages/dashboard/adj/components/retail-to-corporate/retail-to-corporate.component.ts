...
@Component({
  selector: 'app-retail-to-corporate',
  templateUrl: './retail-to-corporate.component.html',
  styleUrls: ['./retail-to-corporate.component.scss']
})
export class RetailToCorporateComponent implements OnInit, OnDestroy {
  ...
  //  console.log(data.value);
    ...
    this.adjService.
      createAdj(this.form.value.pnrDetails, "flight/booking/retail-corporate-adjustMents/" + this.bookingId + "/" + this.supplierReference + "/new-customer-contact/" + contactId + "/"+customerId).pipe(takeUntil(this.ngDestroy$)).subscribe(res => {
      //  console.log(res);
        if (res.status === 200) {
          alert(res.message);
        } else {

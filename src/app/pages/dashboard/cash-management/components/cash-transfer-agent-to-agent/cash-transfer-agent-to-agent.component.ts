...
import { Agent } from 'app/shared/models/agent-api-response';
...
@Component({
  selector: 'app-cash-transfer-agent-to-agent',
  templateUrl: './cash-transfer-agent-to-agent.component.html',
  styleUrls: ['./cash-transfer-agent-to-agent.component.scss'],
})
export class CashTransferAgentToAgentComponent implements OnInit , OnDestroy{
  ...
  noAgentResults: boolean;
  searchAgentTerm: string;
  searchAgentResult: Agent[];
  agentFormatter = (agent: Agent) => agent.fullName;
  ...
  ) { this.titleService.setTitle('Cash Transfer Agent To Agent'); }
   ...
   * data for from input
   */
  onSearchAgent: OperatorFunction<string, readonly { fullName }[]> = (text$: Observable<string>) =>
    text$.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      tap((term: string) => (this.searchAgentTerm = term)),
      switchMap((term) =>
        term.length >= 3
          ? this.masterApiServices.getAgentByName(CASHMANAGEMENT_URLS.SEARCH_URL.AGENT_URL, term).pipe(
              tap((response: Agent[]) => {
                this.noAgentResults = response.length === 0;
                if (this.noAgentResults) {
                  this.toastr.info(`no Agent found given   ${term}`, 'INFO');
                }
                this.searchAgentResult = [...response];
              }),
              catchError(() => {
                return of([]);
              })
      ...
      tap(() =>
        this.searchAgentTerm === '' || this.searchAgentTerm.length <= 2
          ? []
          : this.searchAgentResult.filter(
              (v) => v.fullName.toLowerCase().indexOf(this.searchAgentTerm.toLowerCase()) > -1
            )
      )
    );
  ...
  initializeForm() {
    this.transferAgentForm = this.fb.group({
      bankTransactionType: '',
 ...
 /**
   ...
   */
  get f() {
    return this.transferAgentForm.controls;
  }
  reset(){
    this.transferAgentForm.reset();
    this.submitted = false;
  }

  onSubmitForm(){
    this.submitted = true;
    if (this.transferAgentForm.invalid) {
      return this.toastr.error('Please fill the required fields and submit the form', 'Error');
    }
    if(typeof this.transferAgentForm.value.transferToId !== 'object'){
      return this.toastr.error('Please select agent name ', 'Error');
    }

    if(this.transferAgentForm.valid){
     const saveData={
      bankTransactionType: this.transferAgentForm.value.bankTransactionType,
      referenceNo: this.transferAgentForm.value.referenceNo,
      transferAmount: Number(this.transferAgentForm.value.transferAmount),
      transferFromBankInfo: this.transferAgentForm.value.transferFromBankInfo,
      transferFromId: this.authService.getUser(),
      transferToBankInfo: this.transferAgentForm.value.transferToBankInfo,
      transferToId: this.transferAgentForm.value.transferToId?.userId,
      remarks:this.transferAgentForm.value.remarks
     };

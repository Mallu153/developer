...
@Component({
  selector: 'app-cash-deposit-agent-to-bank',
  templateUrl: './cash-deposit-agent-to-bank.component.html',
  styleUrls: ['./cash-deposit-agent-to-bank.component.scss']
})
export class CashDepositAgentToBankComponent implements OnInit , OnDestroy{
...
constructor(
  ...
  ) { this.titleService.setTitle('Cash Deposit Agent To Bank'); }
...
initializeForm() {
  this.depositAgentForm = this.fb.group({
    bankTransactionType: '',
 ...
 */
get f() {
  return this.depositAgentForm.controls;
}

reset(){
  this.depositAgentForm.reset();
  this.submitted = false;
}

onSubmitForm(){
  this.submitted = true;
  if (this.depositAgentForm.invalid) {
    return this.toastr.error('Please fill the required fields and submit the form', 'Error');
  }
  if(typeof this.depositAgentForm.value.transferToId !== 'object'){
    return this.toastr.error('Please select account details ', 'Error');
  }

  if(this.depositAgentForm.valid){
   const saveData={
    bankTransactionType: 'ToBank',
    referenceNo: this.depositAgentForm.value.referenceNo,
    transferAmount: Number(this.depositAgentForm.value.transferAmount),
    transferFromId:this.authService.getUser(),
    transferFromBankInfo: " ",
    transferToBankInfo: this.depositAgentForm.value.transferToId?.bankInfo,
    transferToId:  this.depositAgentForm.value.transferToId?.id,
    remarks:this.depositAgentForm.value.remarks
   };
